...
class DE_Global_DataLoader(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        outputs = []
        obs = torch.tensor([seq.Pose[i] for i in range(0, self.args.input, self.args.skip)])[:, :2]
        obs_speed = (obs[1:] - obs[:-1])
        true = torch.tensor([seq.Future_Pose[i] for i in range(0, self.args.output, self.args.skip)])[:, :2]
