...
def train(train_loader, val_loader, model, optimizer, scheduler, opt):
    ...
    for epoch in range(opt.epochs):
        ...
        print('e:', epoch, '| train_speed_loss: %.4f' % avg_epoch_train_speed_loss.avg,
              '| validation_speed_loss: %.4f' % avg_epoch_val_speed_loss.avg,
              '| ade_train: %.4f' % ade_train.avg,
              '| ade_val: %.4f' % ade_val.avg, '| fde_train: %.4f' % fde_train.avg, '| fde_val: %.4f' % fde_val.avg,
              '| epoch_time.avg:%.4f' % (time.time() - start))
    print('*' * 100)
